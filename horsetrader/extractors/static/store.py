...
``holiday-golden-week-*``, ``holiday-marketing-*``, ``anniversary-*``,
``scenario-NN``, ``story-NNN``, ``<id>-banner``, etc.), so each loader's pattern
claims a disjoint slice and a key appearing in two files fails loud at merge
time.
