...
test("project folds streams from the snapshot forward into a queryable balance series", () => {
  ...
  const { series } = project(snapshot, [{ stream: "events", emissions: eventStream(bundle, cal(snapshot.date)) }]);
...
test("the ledger keeps per-source attribution for every contributing event", () => {
  ...
  const { ledger } = project(snapshot, [{ stream: "events", emissions: eventStream(bundle, cal(snapshot.date)) }]);
...
test("adding a stream is a change to the input list, not the fold; balances combine across streams", () => {
  ...
  const events = { stream: "events", emissions: eventStream(bundle, cal(snapshot.date)) };
  const spends = { stream: "spends", emissions: [{ date: cal("2026-06-10"), source: "banner-1", deltas: { free_carats: -150 } }] };
