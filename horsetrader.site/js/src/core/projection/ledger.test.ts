...
test("attribute flattens emissions into single-resource entries tagged with stream + source", () => {
  const ledger = attribute("events", [emit("2026-06-10", "banner-1", { free_carats: 720, trainee_tickets: 2 })]);
  assert.deepEqual(ledger, [
    { date: "2026-06-10", stream: "events", source: "banner-1", resource: "free_carats", amount: 720 },
...
test("subtotals sum entries per resource per date, sparse over dates with entries", () => {
  const ledger = [
    ...attribute("a", [emit("2026-06-10", "x", { free_carats: 100 })]),
    ...attribute("b", [emit("2026-06-10", "y", { free_carats: 50, trainee_tickets: 1 })]),
    ...attribute("a", [emit("2026-06-12", "z", { free_carats: 30 })]),
  ];
