...
test("a generator emits its payload once a day from start for repeat days, then stops", () => {
  const out = generatorStream([spec("daily", "2026-06-09", 3, { carats_free: 50 })], "2026-06-01");
  assert.deepEqual(out, [
    { date: "2026-06-09", source: "daily", deltas: { carats_free: 50 } },
    { date: "2026-06-10", source: "daily", deltas: { carats_free: 50 } },
    { date: "2026-06-11", source: "daily", deltas: { carats_free: 50 } },
  ]);
});

test("a generator straddling the snapshot keeps its later days — cadence is computed from start", () => {
  const out = generatorStream([spec("daily", "2026-05-30", 5, { carats_free: 50 })], "2026-06-01");
...
test("a generator stepping over a month boundary advances the calendar correctly", () => {
  const out = generatorStream([spec("daily", "2026-06-29", 4, { carats_free: 10 })], "2026-06-01");
  assert.deepEqual(out.map((e) => e.date), ["2026-06-29", "2026-06-30", "2026-07-01", "2026-07-02"]);
});

test("generatorsFromBundle extracts inline generators, normalising carats and splitting off repeat", () => {
    ...
    events: [
      {
        ...
        rewards: { generator: { carats: 564, repeat: 10 } },
      } satisfies AnchorRecord,
    ],
  };
  assert.deepEqual(generatorsFromBundle(bundle), [
    { source: "anchor-golden-week-2021", start: "2025-08-07", payload: { carats_free: 564 }, repeat: 10 },
  ]);
});
...
test("a malformed generator (no payload, or non-numeric repeat) is skipped", () => {
  const bundle: EventsBundle = {
    events: [
      ...
      { type: "anchor", start: "2025-08-08", end: "2025-08-08", predicted: false, key: "flat-only", rewards: { carats: 100 } } satisfies AnchorRecord,
    ],
  };
  assert.deepEqual(generatorsFromBundle(bundle), []);
});
...
test("the bundle generator round-trips through extraction into a daily emission run", () => {
  const bundle: EventsBundle = {
    events: [
      { type: "anchor", start: "2026-06-09", end: "2026-06-09", predicted: false, key: "anchor-x", rewards: { generator: { carats: 564, repeat: 10 } } } satisfies AnchorRecord,
    ],
  };
  ...
  assert.deepEqual(out[0]?.deltas, { carats_free: 564 });
});
