...
test("a sequence emits per consecutive day from start, skipping null (unpaid) days", () => {
  const out = sequenceStream([spec("login", "2026-06-09", "free_carats", [150, null, 150])], cal("2026-06-01"));
  assert.deepEqual(out, [
    { date: "2026-06-09", source: "login", deltas: { free_carats: 150 } },
    { date: "2026-06-11", source: "login", deltas: { free_carats: 150 } },
  ]);
});

test("a null day advances the calendar but pays nothing — later days stay aligned", () => {
  const out = sequenceStream([spec("login", "2026-06-09", "free_carats", [150, 150, null, 150])], cal("2026-06-01"));
...
test("a sequence straddling the snapshot keeps its later days — cadence is computed from start", () => {
  const out = sequenceStream([spec("login", "2026-05-30", "free_carats", [150, 150, 150, 150, 150])], cal("2026-06-01"));
...
test("the bundle sequence round-trips through extraction into a daily emission run", () => {
  ...
  assert.deepEqual(out[0]?.deltas, { free_carats: 150 });
  assert.equal(out.reduce((sum, e) => sum + (e.deltas["free_carats"] ?? 0), 0), 1500);
});
