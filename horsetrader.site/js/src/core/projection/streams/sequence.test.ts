...
test("a sequence emits per consecutive day from start, skipping null (unpaid) days", () => {
  const out = sequenceStream([spec("login", "2026-06-09", "free_carats", [150, null, 150])], "2026-06-01");
...
test("a null day advances the calendar but pays nothing — later days stay aligned", () => {
  const out = sequenceStream([spec("login", "2026-06-09", "free_carats", [150, 150, null, 150])], "2026-06-01");
...
test("a sequence straddling the snapshot keeps its later days — cadence is computed from start", () => {
  const out = sequenceStream([spec("login", "2026-05-30", "free_carats", [150, 150, 150, 150, 150])], "2026-06-01");
