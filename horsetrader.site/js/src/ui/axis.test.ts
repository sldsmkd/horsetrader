...
test("xForDate is true-to-date: origin is 0, days scale linearly, before-origin is negative", () => {
  ...
  assert.equal(axis.xForDate("2026-06-03"), 0); // the origin sits at content-x 0
  assert.equal(axis.xForDate("2026-06-04"), 10); // one day on
  assert.equal(axis.xForDate("2026-06-13"), 100); // ten days on
  assert.equal(axis.xForDate("2026-06-01"), -20); // two days before the origin
});

test("xForDate counts real calendar gaps, not adjacency (month and year boundaries, UTC)", () => {
  const feb = createAxis({ origin: "2026-02-27", pxPerDay: 10 });
  assert.equal(feb.xForDate("2026-03-02"), 30); // 27→28→Mar1→Mar2 = 3 days (Feb 2026 = 28d)

  const nye = createAxis({ origin: "2025-12-31", pxPerDay: 10 });
  assert.equal(nye.xForDate("2026-01-02"), 20); // across the year boundary
});
...
test("date → x → date round-trips exactly", () => {
  ...
  for (const date of ["2026-06-03", "2026-06-04", "2026-07-01", "2026-05-20", "2027-01-01"]) {
    assert.equal(axis.dateForX(axis.xForDate(date)), date);
  }
});

test("pxPerDay is the zoom: the same date maps proportionally to the scale", () => {
  const tight = createAxis({ origin: "2026-06-03", pxPerDay: 4 });
