...
const ACADEMY: Academy = {
  ...
  supports: {
    ...
    "s-sr": { character: "char-suzuka", display: "Aaa Support", type: "speed", rarity: "sr", title: null, release: "2021", thumbnail: null, art: null, aliases: [], source: null },
  },
  trainees: {
...
test("reserve: zero pity touches nothing — the predicted balance passes through", () => {
  ...
  assert.deepEqual(reserve(ctx, 0, false), { freePulls: 10, freeCarats: 100000, paidCarats: 5000, tickets: 300 });
});
...
test("reserve: cost-ascending — free pulls, then tickets, then daily paid, then free carats", () => {
  ...
  assert.deepEqual(reserve(ctx, 1, false), { freePulls: 0, freeCarats: 100000, paidCarats: 5000, tickets: 110 });
  ...
  assert.deepEqual(reserve(ctx, 2, false), { freePulls: 0, freeCarats: 87400, paidCarats: 4700, tickets: 0 });
});
...
test("reserve: paid carats only leave through the daily window — leftover banks, never drained at full price", () => {
  ...
  const ctx = commitContext(bundle(), "banner-t", { balanceAt: () => BALANCE, commitments: {} });
  assert.deepEqual(reserve(ctx, 5, false), { freePulls: 0, freeCarats: -145800, paidCarats: 300, tickets: 0 });
});
