...
test("identity menu toggles the whole identity family", () => {
  ...
  assert.deepEqual(open, { surface: "identity", stagedPlayStyle: null, stagedPlayStyleSettings: null });

  const withPlayStyle: PlayStyleMachineState = { surface: "playstyle", stagedPlayStyle: "casual", stagedPlayStyleSettings: null };
  assert.deepEqual(reducePlayStyleMachine(withPlayStyle, { type: "toggle-identity" }, saved), PLAY_STYLE_MACHINE_INITIAL);

  const withOshi: PlayStyleMachineState = { surface: "oshi", stagedPlayStyle: null, stagedPlayStyleSettings: null };
  assert.deepEqual(reducePlayStyleMachine(withOshi, { type: "toggle-identity" }, saved), PLAY_STYLE_MACHINE_INITIAL);

  const withPlayStyleOshi: PlayStyleMachineState = { surface: "playstyle-oshi", stagedPlayStyle: "casual", stagedPlayStyleSettings: null };
  assert.deepEqual(
...
test("previewing an unsaved preset opens the playstyle page and enables staging", () => {
  ...
  assert.deepEqual(state, { surface: "playstyle", stagedPlayStyle: "casual", stagedPlayStyleSettings: null });
...
test("clicking the saved preset first reverts an unsaved preview, then closes the page", () => {
  const casual: PlayStyleMachineState = { surface: "playstyle", stagedPlayStyle: "casual", stagedPlayStyleSettings: null };
  const reverted = reducePlayStyleMachine(casual, { type: "preview-playstyle", key: saved }, saved);
  assert.deepEqual(reverted, { surface: "playstyle", stagedPlayStyle: null, stagedPlayStyleSettings: null });
...
test("opening the saved preset stores no redundant staged value", () => {
  ...
  assert.deepEqual(state, { surface: "playstyle", stagedPlayStyle: null, stagedPlayStyleSettings: null });
...
test("discarding or committing returns to the trainer card with no staged preset", () => {
  const state: PlayStyleMachineState = { surface: "playstyle", stagedPlayStyle: "dedicated", stagedPlayStyleSettings: null };
...
test("commit-playstyle-stay clears staging but keeps the play-style page open (custom tweaker)", () => {
  const state: PlayStyleMachineState = { surface: "playstyle", stagedPlayStyle: "custom", stagedPlayStyleSettings: null };
...
test("previewing custom opens the playstyle page like any other preset", () => {
  ...
  assert.deepEqual(state, { surface: "playstyle", stagedPlayStyle: "custom", stagedPlayStyleSettings: null });
...
test("custom as saved preset follows the same toggle logic as other presets", () => {
  ...
  assert.deepEqual(open, { surface: "playstyle", stagedPlayStyle: null, stagedPlayStyleSettings: null });

  const closed = reducePlayStyleMachine(open, { type: "preview-playstyle", key: "custom" }, savedCustom);
  assert.deepEqual(closed, { surface: "identity", stagedPlayStyle: null, stagedPlayStyleSettings: null });
});
...
test("oshi selector is modal over the current identity surface", () => {
  ...
  assert.deepEqual(trainerOshi, { surface: "oshi", stagedPlayStyle: null, stagedPlayStyleSettings: null });
  ...
  const playStyleOshi = reducePlayStyleMachine(
    { surface: "playstyle", stagedPlayStyle: "casual", stagedPlayStyleSettings: null },
    { type: "open-oshi" },
    saved,
  );
  assert.deepEqual(playStyleOshi, { surface: "playstyle-oshi", stagedPlayStyle: "casual", stagedPlayStyleSettings: null });
...
test("club selector is modal over the current identity surface", () => {
  ...
  assert.deepEqual(trainerClub, { surface: "club", stagedPlayStyle: null, stagedPlayStyleSettings: null });
  ...
  assert.deepEqual(playStyleClub, { surface: "playstyle-club", stagedPlayStyle: "casual", stagedPlayStyleSettings: null });
