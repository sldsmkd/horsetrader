...
test("starts idle; set merges a patch and exposes it", () => {
  ...
  assert.deepEqual(store.get(), { selection: null, search: "", bookmarks: false, right: null, resourcesEditing: false });
  store.set({ right: "resources" });
  assert.deepEqual(store.get(), { selection: null, search: "", bookmarks: false, right: "resources", resourcesEditing: false });
});
