/**
 * unity-sync — Unity's cloud Worker. This first cut is AUTH ONLY: the Google
 * OAuth2 authorization-code round-trip + a stateless signed-cookie session.
 * R2 plan sync (`/api/sync`) lands in this same Worker next.
 *
...
<script>
async function me(){const r=await fetch('/api/me',{credentials:'include'});
 document.getElementById('out').textContent=JSON.stringify(await r.json(),null,2);}
async function logout(){await fetch('/api/auth/logout',{method:'POST',credentials:'include'});me();}
me();
</script>`;
