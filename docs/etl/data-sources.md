...
| [`config/yaml/christmas.yaml`](../../config/yaml/christmas.yaml) | **Consolidated.** | Reward-bearing Christmas campaigns keyed `holiday-christmas-YYYY`. Empty campaign phases are omitted; modellable phases carry a JP window, shared rewards/banner, and an optional confirmed EN overlay. Read by `extractors/static/christmas.py` into `Holiday` events. |
| [`config/yaml/new_year.yaml`](../../config/yaml/new_year.yaml) | **Consolidated.** | New Year holiday records keyed `holiday-new-year-YYYY`, with nested `countdown:` lead-ins and optional phase-specific `banner:` art. Main events and countdowns bake as `Holiday` records; countdown EN windows are derived from the main New Year launch when needed. |
...
| [`config/yaml/banners.yaml`](../../config/yaml/banners.yaml) | **Consolidated.** | EN confirmed banner periods, keyed by `<id>-banner`, each under an `en:` block with FQ ISO `start` / `end` (22:00 UTC). Read by `extractors/static/banners.py` (via the merged store); stamps a UTC `Period` onto the matching `Banner` at extraction time. A banner absent here stays predicted unless its documented JP-only occurrence carries `predictable: false`. |
...
`golden_week.yaml`, `christmas.yaml`, `new_year.yaml`, `marketing.yaml`, `stories.yaml`,
